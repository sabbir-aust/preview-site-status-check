...
urls.forEach(({ country, url }) => {
  test(`has title and checks status code for Acuiti ${country} URL`, async ({ page }) => {
    ...
    responses.push({ url: mainUrl, status: 200 });
    ...
    await saveResponsesToExcel(responses, country, 'ACUITI');
      ...
      await logFailure(country, mainUrl, error.message, statusCode, 'ACUITI');
    } finally {
      // If no entries were recorded, log an unknown status entry
      if (responses.length === 0) {
        await logFailure(country, mainUrl, 'No response recorded', 404, 'ACUITI');
      }
    }
  });
});