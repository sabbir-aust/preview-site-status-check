...
urls.forEach(({ country, url }) => {
  test(`has title and checks status code for Abbviepro Medical  ${country} URL`, async ({ page }) => {
    ...
    await saveResponsesToExcel(responses, country, 'Medical');
      ...
      await logFailure(country, mainUrl, error.message, statusCode, 'Medical');
    } finally {
      // If no entries were recorded, log an unknown status entry
      if (responses.length === 0) {
        await logFailure(country, mainUrl, 'No response recorded', 404, 'Medical');
      }
    }
  });
});