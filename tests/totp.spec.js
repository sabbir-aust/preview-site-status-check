...
urls.forEach(({ country, url }) => {
  test(`has title and checks status code for TOTP ${country} URL`, async ({ page }) => {
    ...
    try {
      ...
      await saveResponsesToExcel(responses, country, 'TOTP');
      ...
      await logFailure(country, mainUrl, error.message, statusCode, 'TOTP');
    } finally {
      // If no entries were recorded, log an unknown status entry
      if (responses.length === 0) {
        await logFailure(country, mainUrl, 'No response recorded', 404, 'TOTP');
      }
    }
  });
});
